...
(defun windowed-fft! (signal-vector window-dest dest center length &optional (window-fn 'hann))
...
index, multiplied by a window generated by the chosen window function"
  ...
  (let* ((input-window (extract-centered-window-into signal-vector center length window-dest))
    ...
    (loop for index from 0 below length do
          (setf (aref input-window index)
                (* (aref input-window index)
                   (aref window index))))
    (unless (and *fft-instance*
                 (= (size *fft-instance*) length))
      (setf *fft-instance* (make-fft-instance length)))
    (fft-common *fft-instance* input-window dest)))
...
index, multiplied by a window generated by the chosen window function"
  ...
  (unless (typep signal-vector 'complex-sample-array)
    (setf signal-vector (convert-to-complex-sample-array signal-vector)))  
  (let* ((element-type (array-element-type signal-vector))
         (input-window (make-array length
                            :element-type 'complex-sample 
                            :adjustable nil
                            :fill-pointer nil))
         (dest (make-array length
                            :element-type 'complex-sample 
                            :adjustable nil
                            :fill-pointer nil)))
    (declare (type complex-sample-array input-window))
    (windowed-fft! signal-vector input-window dest center length window-fn)
    dest))
